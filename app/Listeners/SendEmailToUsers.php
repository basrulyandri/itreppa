...
class SendEmailToUsers
{
    ...
    public function handle(FormDownloadBrosurEvent $event)
    {
        ...
        if($aplikan->tanggal_take != NULL){
            ...
            if(is_numeric($aplikan->sumber_informasi)){

                $referrer = \App\User::find($aplikan->sumber_informasi);
                Mail::send('layouts.emails.notificationdownloadrosurtoreferrer',compact(['aplikan']),function($message) use ($referrer,$aplikan){
                    $message->from(getOption('email_from'),getOption('email_from_label'))
                        ->to($referrer->email)                    
                        ->subject('Aplikan referensimu baru saja download brosur ('.$aplikan->nama.')');
                });
            }
