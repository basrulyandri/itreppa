...
class Aplikan extends Model
{
    ...
    {
        return $this->hasMany(AplikanTrack::class);
    }
    ...
    {
        return $this->belongsToMany(User::class,'followup','aplikan_id','user_id')->withPivot('keterangan','type')->withTimestamps();
    }

    public function followup()
    {
        return $this->hasMany(followup::class,'aplikan_id');
    }
    ...
    {
        return $this->followup()->whereType('followup')->count();
    }

    public function jmlDilayani()
    {
        return $this->followup()->whereType('melayani')->count();
    }
    ...
    public function isMyReference($userId)
    {
        if($this->sumber_informasi != $userId){
            return false;
        }
        return true;
    }
    ...
    {
        if(AplikanTrack::whereAplikanId($this->id)->count() > 0 || $this->hasBeenTaken()){
            return true;
        }
        return false;
    }
    ...
    {
        if(is_numeric($this->sumber_informasi)){
            return User::find($this->sumber_informasi)->username;
        }

        return $this->sumber_informasi;
    }
